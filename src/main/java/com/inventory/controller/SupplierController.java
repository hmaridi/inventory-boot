...
public class SupplierController {
	...
	ResponseEntity<Supplier> findOneSupplier(@PathVariable Long id) {
		return this.supplierRepository.findById(id).map(ResponseEntity::ok)
	...
	ResponseEntity<Supplier> saveSupplier(@RequestBody Supplier s) {
		...
		URI uri = MvcUriComponentsBuilder.fromController(getClass()).path("/{id}").buildAndExpand(supplier.getId())
				.toUri();
	...
	ResponseEntity<?> deleteSupplier(@PathVariable Long id) {
		return this.supplierRepository.findById(id).map(c -> {
	...
	ResponseEntity<?> existSupplier(@PathVariable Long id) {
		return this.supplierRepository.findById(id).map(exists -> ResponseEntity.noContent().build())
				.orElseThrow(() -> new SupplierNotFoundException(id));
	}
	...
	ResponseEntity<Supplier> saveSupplier(@PathVariable Long id, @RequestBody Supplier s) {
		return this.supplierRepository.findById(id).map(existing -> {
			s.setId(id);
			Supplier supplier = this.supplierRepository.save(s);
